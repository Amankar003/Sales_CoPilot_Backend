...
def get_dashboard_stats(session: Session = Depends(get_session)) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.warning(f"Failed to query businesses: {repr(e)}")
        total_leads = 0

    try:
        total_audits = session.exec(select(func.count(Audit.id))).one()
    except Exception as e:
        logger.warning(f"Failed to query audits: {repr(e)}")
        total_audits = 0

    try:
        total_reports = session.exec(select(func.count(Report.id))).one()
    except Exception as e:
        logger.warning(f"Failed to query reports: {repr(e)}")
        total_reports = 0

    try:
        total_campaigns = session.exec(select(func.count(Campaign.id))).one()
    except Exception as e:
        logger.warning(f"Failed to query campaigns: {repr(e)}")
        total_campaigns = 0
    ...
    try:
        ...
        logger.warning(f"Failed to query email logs: {repr(e)}")
        emails_sent = 0
    ...
    try:
        ...
        logger.warning(f"Failed to calculate avg audit score: {repr(e)}")
        avg_audit_score = 0

    # Calculate leads per campaign
    leads_per_campaign = 0
    if total_campaigns > 0:
        leads_per_campaign = round(total_leads / total_campaigns, 1)

    return {
        "total_campaigns": total_campaigns,
        "total_leads": total_leads,
        "leads_per_campaign": leads_per_campaign,
...
def get_recent_leads(
    ...
    try:
        ...
        logger.warning(f"Failed to fetch recent leads: {repr(e)}")
        return []
...
def get_recent_campaigns(
    ...
    """Get the most recently created campaigns."""
    try:
        ...
        logger.warning(f"Failed to fetch recent campaigns: {repr(e)}")
        return []
...
def get_recent_audits(
    ...
    try:
        ...
        logger.warning(f"Failed to fetch recent audits: {repr(e)}")
        return []
